...
class Reconsile extends React.Component {
  timer = null
  ...
  getLogData(page) {
    let data2send = this.state.logrecon
    data2send['page'] = page || 1
    Api.get('/monitoring/reconsile', {params: data2send}).then(res => {
      this.setState({
        logs: res.data,
        page: {
  ...
  handleChange(e) {
    let logrecon = this.state.logrecon
    logrecon[e.target.name] = e.target.value
    this.setState({
      logrecon: logrecon
    })
      ...
      <div className="animated fadeIn">
        <div className="row">
          <div className="col-md-12">
            <div className="card">
              <div className="card-header">Log Reconsile</div>
...
export default connect(mapStateToProps, mapDispatchToProps)(Reconsile)
